...
int main() {
    ...
    int row_index, col_index, i, j;
    S_and_C s_and_c;
    Matrix *m = generate_symmetric_matrix(4);
    print_matrix(m);
    MaxElement *max_element = matrix_get_non_diagonal_max_element(m);
    s_and_c = get_s_and_c_for_rotation_matrix(m, max_element);
    i = max_element_get_index1(max_element), j = max_element_get_index2(max_element);
    space();
    print_max_element(max_element);
    space();
    printf("s: %f c: %f", s_and_c.s, s_and_c.c);
    space();
    Matrix *mm = transform_matrix(m, s_and_c, max_element);
    print_matrix(mm);
    free(mm);
    free(max_element);
    free_matrix(m);
